export const FETCH_CHARACTERS_START = '[Characters] Fetch start';
export const FETCH_CHARACTERS_SUCCESS = '[Characters] Fetch success';
export const FETCH_CHARACTERS_ERROR = '[Characters] Fetch error';

export const FETCH_CHARACTER_DETAILS_START = '[Character Details] Fetch start';
export const FETCH_CHARACTER_DETAILS_SUCCESS =
  '[Character Details] Fetch success';
export const FETCH_CHARACTER_DETAILS_ERROR = '[Character Details] Fetch error';

export const UPDATE_FILTERS = '[Filters] Update';

export const SELECT_PAGE = '[Pagination] Select page';
export const SELECT_LIMIT = '[Pagination] Select limit';
export const UPDATE_PAGINATION = '[Pagination] Update';
